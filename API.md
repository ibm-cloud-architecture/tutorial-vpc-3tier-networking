...
```
curl -X GET "$RIAS_ENDPOINT/v1/regions/us-south/zones?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
Result
```
...
```
curl -X GET "$RIAS_ENDPOINT/v1/instances?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
will give the list of all the VSIs and their IDs. Then you can use `<object_id>`
```
curl -X GET "$RIAS_ENDPOINT/v1/instances/<object_id>?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
...
```
curl -X POST "$RIAS_ENDPOINT/v1/keys?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"vpc-key\", \
           \"public_key\" : \"$SSH_KEY\", \
           \"type\" : \"rsa\" \
...
```
curl -X POST "$RIAS_ENDPOINT/v1/vpcs/$VPC/address_prefixes?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"  \
...
**Subnet1**
```
curl -X POST "$RIAS_ENDPOINT/v1/subnets?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"subnet1\", \
...
```
curl -X GET "$RIAS_ENDPOINT/v1/subnets/$SUBNET2?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
Result
```
...
```
curl -X GET "$RIAS_ENDPOINT/v1/instance/profiles?version=$API_VERSION&generation=2&limit=2" \
     -H "Authorization: Bearer $iam_token"
```
Result
```
...
```
curl -X GET "$RIAS_ENDPOINT/v1/images?version=$API_VERSION&generation=2&limit=2&start=3" \
     -H "Authorization: Bearer $iam_token"
```
Result
```
...
```
curl -X POST "$RIAS_ENDPOINT/v1/security_groups?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"app_sg\", \
           \"vpc\" : { \"id\": \"$VPC\" } \
         }"
...
**Instance = MySQL1**
```
curl -X POST "$RIAS_ENDPOINT/v1/instances?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"MySQL1\", \
           \"vpc\" : { \"id\" : \"$VPC\" }, \
           \"zone\" : { \"name\" : \"$ZONE\" }, \
...
**Load Balancer = LB1**
```
curl -X POST "$RIAS_ENDPOINT/v1/load_balancers?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"LB1\", \
           \"is_public\" : true, \
           \"subnets\" : [ { \"id\" : \"$SUBNET1\" } ], \
...
```
curl -X GET "$RIAS_ENDPOINT/v1/load_balancers/$LB1?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
Result
```
...
```
curl -X POST "$RIAS_ENDPOINT/v1/load_balancers/$LB1/pools?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"pool1\", \
           \"protocol\" : \"http\", \
...
```
curl -X POST "$RIAS_ENDPOINT/v1/load_balancers/$LB1/pools/$POOL1/members?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"port\" : 80, \
...
```
curl -X POST "$RIAS_ENDPOINT/v1/load_balancers/$LB1/listeners?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"port\" : 80, \
           \"protocol\" : \"http\", \
...
**FIP = app1fip**
```
curl -X POST "$RIAS_ENDPOINT/v1/floating_ips?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"app1fip\", \
           \"zone\" : { \"name\" : \"$ZONE\" } \
     }"
...
```
curl -X PATCH "$RIAS_ENDPOINT/v1/floating_ips/$APP1FIP?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
...
```
curl -X PATCH "$RIAS_ENDPOINT/v1/floating_ips/$APP2FIP?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
...
```
curl -X PATCH "$RIAS_ENDPOINT/v1/floating_ips/$DATA1FIP?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
...
```
curl -X PATCH "$RIAS_ENDPOINT/v1/floating_ips/$DATA2FIP?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
...
```
curl -X POST "$RIAS_ENDPOINT/v1/public_gateways?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"name\" : \"vpc_pub_gw\", \
           \"vpc\" : { \"id\" : \"$VPC\" }, \
           \"zone\" : { \"name\" : \"$ZONE\" } \
...
**Subnet1**
```
curl -X PUT "$RIAS_ENDPOINT/v1/subnets/$SUBNET1/public_gateway?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"id\" : \"$PUBGW\" }"
```
Result
...
**Subnet2**
```
curl -X PUT "$RIAS_ENDPOINT/v1/subnets/$SUBNET2/public_gateway?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"id\" : \"$PUBGW\" }"
```
Result
...
```
curl -X POST "$RIAS_ENDPOINT/v1/security_groups/$APP_SG/rules?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"direction\" : \"inbound\", \
           \"protocol\" : \"tcp\", \
           \"port_min\" : 22, \
...
**Add an inbound rule to allow all tcp access on port 22 for SSH access to the VSIs.**
```
curl -X POST "$RIAS_ENDPOINT/v1/security_groups/$DATA_SG/rules?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token" \
     -d "{ \"direction\" : \"inbound\", \
           \"protocol\" : \"tcp\", \
           \"port_min\" : 22, \
...
```
curl -v -X DELETE "$RIAS_ENDPOINT/v1/instances/$APPSERV1/network_interfaces/$APPSERV1_NIC0/floating_ips/$APP1FIP?version=$API_VERSION&generation=2" \
     -H "Authorization: Bearer $iam_token"
```
...
```
curl -X DELETE "$RIAS_ENDPOINT/v1/floating_ips/$APP1FIP?version=$API_VERSION&generation=2 \
